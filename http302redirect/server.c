...
void main(int argc ,char **argv)
{
	...
	if (create_send_str() != 0)
	{
		perror("Creating send_str failed.");
        exit(1);
	}
    ...
    while (1)
    {
        ...
        {                       //检测是否有新客户端请求
			...
			//bzero(&sin_size, sizeof(sin_size));
...
{
	if((!strncmp(recvbuf, "GET", 3)) || (!strncmp(recvbuf, "POST", 4)))
	{
...
/*************************************************
* Function    : create_send_str()
* Description : 生成要发送的字符串
* Calls       : 
* Called By   : main()
* Input       : 
* Output      : 
...
int create_send_str()
{
	...
	if ((fp = popen("ifconfig br-lan | grep 'inet addr' | awk '{print $2}' | awk -F: '{print $2}'","r")) == NULL)
	{
		perror("popen");
		return -1;
	}
	...
	len = snprintf(sendbuf, sizeof(sendbuf), "%s%s%s%s%s%s",	\
						...
						"Location: http://",	\
						tmpbuf);
	strncpy(&sendbuf[len-1], "/index.html\r\n\r\n", strlen("/index.html\r\n\r\n"));
	printf("%s\n", sendbuf);
	return 0;	
}